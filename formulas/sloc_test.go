...
func TestSLOCZeroGivesZero(t *testing.T) {
	lon1, lat1, lon2, lat2, earthRadius := 0.00, 0.00, 0.00, 0.00, 6371.00
	distance := SphericalLawOfCosines(lat1, lon1, lat2, lon2, earthRadius)
	want := 0.00
	if distance != want {
...
func TestSLOC(t *testing.T) {
	...
	distance := int(math.Round(SphericalLawOfCosines(lat1, lon1, lat2, lon2, earthRadius)))
	want := 17892709
